...
<!--
Схема представляет собой стандартную топологию сети OSPFv2, используемую в этом модуле.  Он состоит из трех маршрутизаторов с интерфейсами в области 0 и один из маршрутизаторов имеет подключение к Интернету. Маршрутизаторы соединены друг с другом, чтобы сформировать треугольник. В левом нижнем углу находится маршрутизатор R1. R1 имеет три используемых интерфейса: Lo0 с IP-адресом .1 в сети 10.10.1.0/24; G0/0/0 с IP-адресом .5 в сети 10.1.1.4/30 подключен к маршрутизатору R2 вверху; и G0/0/1 с IP-адресом .14 в сети 10.1.1.12/30 подключен к маршрутизатору R3 справа. R2 имеет четыре используемых интерфейса: G0/0/0 с IP-адресом .6 в сети 10.1.1.4/30, подключенной к R1; G0/0/1 с IP-адресом .9 в сети 10.1.1.8/30 подключен к маршрутизатору R3; Lo0 с адресом .1 в сети 10.10.2.0/24 отображается с подключением вверху; и Lo1 с IP-адресом .1 на сеть 64.100.0.0/30, которая подключена к интернет-облаку справа. Lo1 и облако Интернета не находятся в области 0. В правом нижнем углу находится R3. R3 имеет три используемых интерфейса: Lo0 с IP-адресом .1 в сети 10.10.3.0/24; G0/0/0 с IP-адресом .13 в сети 10.1.1.12/30, подключенной к R1; и G0/0/1 с IP-адресом .10 в сети 10.1.1.8/30, подключенной к R2.
-->

<!-- 2.1.2 -->
## Режим конфигурации маршрутизатора OSPF

OSPFv2 включается с помощью команды режима глобальной настройки **router ospf** process-id. Значение process-id представляет собой число от 1 до 65535 и выбирается сетевым администратором. Значение process-id имеет только локальное значение, то есть оно не обязательно должно совпадать со значением на других маршрутизаторах OSPF для установления отношений смежности с такими соседними маршрутизаторами. Рекомендуется использовать один и тот же _process-id_ на всех маршрутизаторах OSPF.

После ввода команды **router ospf** _process-id_ маршрутизатор переходит в режим настройки маршрутизатора, как указано знаком **R1(config-router)#** . Введите знако вопроса (**?** ), чтобы просмотреть все команды, доступные в этом режиме. Список команд, показанный здесь, был изменен для отображения только тех команд, которые относятся к этому модулю.
...
<!--
Стандартная топология сети OSPFv2, показывающая идентификатор маршрутизатора для каждого маршрутизатора
-->

Используйте команду режима конфигурации **router-id** _rid_ router, чтобы вручную назначить идентификатор маршрутизатора. В этом примере router ID 1.1.1.1 назначается на R1. Используйте команду **show ip protocols**  для проверки идентификатора маршрутизатора.
