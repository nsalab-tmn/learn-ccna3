...
<!--
На рисунке показаны избыточные связи между устройствами доступа и устройствами основного уровня. В проводном шкафу есть два коммутатора, а в магистрали — четыре. У всех избыточные каналы связи. Кроме того, магистральные имеют избыточные связи с фермой серверов. Она состоит из двух коммутаторов и семи серверов.
-->
...
<!--
На рисунке показано несколько каналов связи между коммутаторами, использующими EtherChannel. Два многослойных переключателя с двумя связями, каждый с коммутатором. Каналы объединяются с помощью EtherChannel.
-->
...
<!--
На рисунке показано использование масштабируемого протокола маршрутизации, такого как OSPF. Три области OSPF: 0, 1 и 51. Область 1 имеет четыре маршрутизатора, один из которых обозначен R1 на границе. Он также находится и в области 0. В ней два маршрутизатора: R1 и R2. R2 также находится и в области 51. Она состоит из четырех маршрутизаторов, подключенных через последовательные каналы.
-->
...
<!--
Рисунок иллюстрирует реализацию беспроводного подключения для поддержки мобильности и расширения. Маршрутизатор R1 соединен с коммутатором S1. У S1 избыточные каналы связи с коммутатором S2. У S2 связи с двумя ПК и точкой беспроводного доступа Cisco. Она подключается по беспроводной сети к мобильному телефону, ноутбуку и планшету.
-->
...
<!--
На рисунке показана область сбоя пограничного маршрутизатора. К нему подключено облако, изображающее Интернет. У пограничного маршрутизатора две разветвленные связи с двумя коммутаторами S1 и S2. У S1 каналы всязи с двумя ПК с надписью H2 и H3 и точкой беспроводного доступа с надписью AP1. У AP1 беспроводное подключение к ноутбуку с надписью H1. У S2 каналы связи с коммутатором с меткой S3 и ПК с меткой H4. У S3 два канала связи с двумя ПК с надписью H5 и H6. Домен сбоя для пограничного маршрутизатора выделяется квадратом, который охватывает все устройства, подключенные к пограничным маршрутизаторам, за исключением связи с Интернетом.
-->
...
<!--
На рисунке показан домен сбоя точки доступа AP1. Облако, изображающее Интернет, подключено к пограничному маршрутизатору. У него две разветвленные связи с двумя коммутаторами S1 и S2. У S1 каналы связи с двумя ПК с надписью H2 и H3 и точкой беспроводного доступа с надписью AP1. У AP1 беспроводное подключение к ноутбуку с надписью H1. У S2 каналы связи с коммутатором с меткой S3 и ПК с меткой H4. У S3 два канала связи с двумя ПК с надписью H5 и H6. Домен сбоя для AP1 состоит только из ПК H1, подключенного к AP1 по беспроводной сети.
-->
...
<!--
На рисунке показана область сбоя S1. Облако, изображающее Интернет, подключено к пограничному маршрутизатору. У него две разветвленные связи с двумя коммутаторами S1 и S2. У S1 каналы связи с двумя ПК с надписью H2 и H3 и точкой беспроводного доступа с надписью AP1. У AP1 беспроводное подключение к ноутбуку с надписью H1. У S2 каналы связи с коммутатором с меткой S3 и ПК с меткой H4. У S3 два канала связи с двумя ПК с надписью H5 и H6. Домен отказа для S1 состоит из всех устройств, подключенных к S1; ПК, H2 и H3, а также AP1 и его беспроводного соединения H1.
-->
...
<!--
На рисунке показана область сбоя S2. Облако, изображающее Интернет, подключено к пограничному маршрутизатору. У него две разветвленные связи с двумя коммутаторами S1 и S2. У S1 каналы связи с двумя ПК с надписью H2 и H3 и точкой беспроводного доступа с надписью AP1. У AP1 беспроводное подключение к ноутбуку с надписью H1. У S2 каналы связи с коммутатором с меткой S3 и ПК с меткой H4. У S3 два канала связи с двумя ПК с надписью H5 и H6. Домен сбоя для S2 состоит из всех устройств, подключенных к S2; ПК, H4 и S3, а также S3s ссылки на ПК H5 и H6.
-->
...
<!--
На рисунке показана область сбоя S3. Облако, изображающее Интернет, подключено к пограничному маршрутизатору. У него две разветвленные связи с двумя коммутаторами S1 и S2. У S1 каналы связи с двумя ПК с надписью H2 и H3 и точкой беспроводного доступа с надписью AP1. У AP1 беспроводное подключение к ноутбуку с надписью H1. У S2 каналы связи с коммутатором с меткой S3 и ПК с меткой H4. У S3 два канала связи с двумя ПК с надписью H5 и H6. Домен сбоя для S3 состоит из всех устройств, подключенных к S3; ПК H5 и H6.
-->
...
<!--
На рисунке показано увеличение полосы пропускания за счет использования EtherChannel и объединения нескольких каналов связи между коммутаторами. Два многоуровневых коммутатора с двумя каналами связи с коммутатором, которые объединены с помощью EtherChannel.
-->
...
<!--
На рисунке показана беспроводная точка доступа Cisco, соединяющая беспроводные устройства с сетью. У маршрутизатора R1 соединение с коммутатором S1. У S1 избыточные каналы связи с коммутатором S2. У S2 связи с двумя ПК и точкой беспроводного доступа Cisco. Она подключается по беспроводной сети к мобильному телефону, ноутбуку и планшету.
-->
...
<!--
На рисунке показано использование протоколов маршрутизации в больших сетях. На рисунке показаны три области OSPF: 0, 1 и 51. У область 1 четыре маршрутизатора, один из которых обозначен R1 на границе. Он также находится и в области 0. В ней два маршрутизатора: R1 и R2. R2 также находится и в области 51. Она состоит из четырех маршрутизаторов, подключенных через последовательные каналы.
-->
