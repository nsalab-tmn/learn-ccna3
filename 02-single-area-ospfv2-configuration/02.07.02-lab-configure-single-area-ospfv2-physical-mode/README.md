...
Часть 2. Настроить и проверить базовую работу протокола OSPFv2 для одной области

Часть 3. Оптимизировать и проверить конфигурацию OSPFv2 для одной области
...
1. На **полке** есть несколько коммутаторов, роутеров и других устройств. Нажмите на роутеры **R1** и **R2** и коммутаторы **S1**, **S2** и перетащите в **стойку**. Нажмите на **Web-Server** и перетащите его в **стойку**. Нажмите на**Laptop** и перетащите его на стол **Table**.

2. Включите роутеры и ноутбук.
...
Осмотрите сетевые подключения. Изначально при подключении устройств к порту коммутатора индикаторы интерфейсов будут желтыми. Примерно через минуту индикаторы станут зелеными.

**Шаг 2. Произведите базовую настройку маршрутизаторов и коммутатора**

1. На монтажной панели **Cable Pegboard** нажмите на **консольный** кабель.
...
4. Отключите поиск DNS, чтобы предотвратить попытки роутера неверно преобразовывать введенные команды таким образом, как будто они являются именами узлов.
...
**Шаг 1. Настройте адреса интерфейса и базового OSPFv2 на каждом роутере**

1. Подключите **консольный** кабель между **R1** и **Laptop**.

2. Настройте адреса интерфейсов на каждом роутере, как показано в **таблице адресации**.

3. Перейдите в режим конфигурации роутера OSPF, используя идентификатор процесса 56.

4. Настройте статический идентификатор для каждого устройства (1.1.1.1 для R1, 2.2.2.2 для R2).
...
7. Переключите консольный кабель к **R2** и повторите подшаги с b до f для **R2**. После настройки R1 и R2 вы можете просто использовать Telnet между ними, чтобы не перемещать консольный кабель каждый раз.

8. Убедитесь, что OSPFv2 работает между роутерами. Выполните команду, чтобы убедиться, что R1 и R2 сформировали смежность.
...
10. Выберите **Laptop \> Command Prompt**, а затем запустите эхо-запрос до **Web server** на адрес 172.16.1.10. После одного или двух тайм-аутов эхо-запрос должен быть успешным. В противном случае устраняйте неполадки физических подключений и конфигурации устройств.

### Часть 3. Оптимизация и проверка конфигурации OSPFv2 для одной области

**Шаг 1. Реализация различных оптимизаций на каждом роутере**

1. На R1 настройте приоритет OSPF интерфейса G0/0/1 на 50, чтобы убедиться, что **R1** является назначенным роутером **DR**.

2. Настройте таймеры OSPF на G0/0/1 каждого устройства для таймера приветствия, составляющего 30 секунд.
...
4. Измените базовую пропускную способность для роутеров. После этой настройки перезапустите OSPF с помощью команды **clear ip ospf process**. Обратите внимание на сообщение консоли после установки новой опорной полосы пропускания.

**Шаг 2. Убедитесь, что оптимизация OSPFv2 реализовалась**

1. Выполните команду **show ip ospf interface g0/0/1** на **R1** и убедитесь, что приоритет интерфейса установлен равным 50, временные интервалы — Hello 30, Dead 120, а тип сети по умолчанию — Broadcast.

2. На **R1** выполните команду **show ip route ospf**, чтобы убедиться, что сеть R2 G0/0/0 присутствует в таблице маршрутизации. Обратите внимание на разницу в метрике между этим выходным и предыдущим.

3. Введите команду **show ip route ospf** на **R2**. Единственной информацией о маршруте OSPF должен быть распространяемый по умолчанию маршрут R1.
