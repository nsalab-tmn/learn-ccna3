...
<!--
Сетевая топология показывает, что хост PC-A с IP 192.168.10.10 и MAC AA:AA:AA:AA:AA:AA,подсоединен к коммутатору, который подсоединен к маршрутизатору с IP 192.168.10.1 и MAC A1:A1:A1:A1:A1:A1. К коммутатору также подключен хост злоумышленника с IP 192.168.10.254 и MAC EE:EE:EE:EE:EE:EE. ARP-кеш на PC-A не имеет записи для 192.168.10.1. ARP-кеш хоста злоумышленника имеет те же самые две записи; IP-адрес 192.168.10.10, связанный с MAC-адресом AA:AA:AA:AA:AA:AA, и IP-адрес 192.168.10.1, связанный с MAC-адресом A1:A1:A1:A1:A1:A1. PC-A посылает ARP-запрос с поиском MAC-адреса хоста 192.168.10.1.
-->
...
<!--
Сетевая топология такая же как и в 3.8.2-1. R1 посылает ARP-ответ, который гласит, что 192.168.10.1 имеет A1:A1:A1:A1:A1:A1. Теперь ARP-кеш на хосте PC-A имеет запись IP-адрес 192.168.10.1 с MAC-адрес A1:A1:A1:A1:A1:A1. Теперь ARP-кеш на хосте PC-A имеет запись IP-адрес 192.168.10.10 с MAC-адрес AA:AA:AA:AA:AA:AA. ARP-кеш хоста злоумышленника имеет те же самые две записи; IP-адрес 192.168.10.10, связанный с MAC-адресом AA:AA:AA:AA:AA:AA, и IP-адрес 192.168.10.1, связанный с MAC-адресом A1:A1:A1:A1:A1:A1.
-->
...
<!--
Сетевая топология такая же как и в 3.8.2-1. Она показывает, что хост злоумышленника послал два ARP-ответа. Один ARP-ответ: 192.168.10.1 имеет EE:EE:EE:EE:EE:EE Второй ARP-ответ: 192.168.10.10 имеет EE:EE:EE:EE:EE:EE. Это приводит к тому, что PC-A меняет запись в ARP-кеш для IP-адреса 192.168.10.1 с MAC-адресом EE:EE:EE:EE:EE:EE и R1 меняет запись в ARP-кеше для IP-адреса 192.168.10.10 с MAC-адресом EE:EE:EE:EE:EE:EE. ARP-кеш хоста злоумышленника имеет теже самые две записи; IP-адрес 192.168.10.10, связанный с MAC-адресом AA:AA:AA:AA:AA:AA, и IP-адрес 192.168.10.1, связанный с MAC-адресом A1:A1:A1:A1:A1:A1.
-->
...
Нажмите "Воспроизведение", чтобы просмотреть видео Спуфинг ARP.

![youtube](https://www.youtube.com/watch?v=3.8.3.mp4)
...
Для предоставления ответов на запросы во многих организациях используются сервисы общедоступных открытых DNS-серверов, например GoogleDNS (8.8.8.8). DNS-сервер такого типа называется открытым преобразователем имен. Открытый преобразователь имен DNS отвечает на запросы от клиентов, находящихся за пределами его административного домена. Открытые преобразователи имен DNS уязвимы для различных вредоносных действий, включая следующие.
...
| **Атаки по методу умножения и отражения на основе DNS**  | Злоумышленники при DoS- или DDoS-атаках используют открытые преобразователи имен DNS, чтобы увеличить объем атаки и скрыть настоящий источник атаки. Злоумышленники отправляют DNS-сообщения открытым преобразователям имен, используя IP-адрес целевого хоста (жертвы). Эти атаки возможны, поскольку открытый преобразователь имен будет отвечать на запросы от любого объекта. |
...
| --- | --- |
| **Метод Fast flux**  | Злоумышленники используют этот метод, чтобы скрыть фишинговые сайты и сайты, распространяющие вредоносное ПО, внутри быстро меняющейся сети взломанных DNS хостов. IP-адреса DNS постоянно изменяются в течение считанных минут. Ботнеты часто используют методы Fast Flux, чтобы эффективно скрыть (т. е. замаскировать) от обнаружения вредоносные серверы. |
| **Метод Double IP Flux**  | Злоумышленники используют этот метод, чтобы быстро изменять сопоставления имен хостов и IP-адресов, а также изменять доверенный сервер имен. В результате повышается сложность определения источника атаки. |
...
При теневом копировании доменов злоумышленник взламывает родительский домен и создает несколько субдоменов для использования во время атаки. Эти субдомены обычно указывают на вредоносные серверы без предупреждения фактического владельца родительского домена.

<!-- 3.8.5 -->
## Туннелирование DNS

Хакеры, использующие туннелирование DNS, помещают в трафик DNS не относящийся к DNS трафик. Этот метод часто позволяет обойти решения по обеспечению безопасности, когда злоумышленник хочет связаться с ботами внутри защищенной сети или отфильтровать данные из организации, такие как база паролей. Для использования хакером туннелирования DNS изменяются различные типы записей DNS. Вот как работает туннелирование DNS для команд CnC, отправляемых в ботнет:
...
<!--
На рисунке показан обмен сообщениями между клиентом и сервером DHCP во время нормальной работы DHCP. Сначала клиент отправляет широковещательный запрос DHCPDISCOVER на сервер, указывая, что он хотел бы запросить адрес. Сервер отвечает одноадресной рассылкой DHCPOFFER сообщения, которое гласит: Я - DHCPsvr1. Вот адрес, который я могу предложить. Сообщение включает в себя IP-адрес 192.168.10.15, маску подсети 255.255.255.0, шлюз по умолчанию 192.168.10.1 и срок аренды 3 дня. Клиент отвечает сообщением DHCPREQUEST, указывающим, что он принимает предложение IP-адреса. Сервер отвечает одноадресной передачей DHCPACK, указывающей, что подтверждение принято.
-->
...
[Исследование трафика DNS (pdf)](./assets/3.8.8-lab---explore-dns-traffic.pdf)
