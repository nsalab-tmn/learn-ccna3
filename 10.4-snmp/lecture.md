...
<!--
Диспетчер SNMP может собирать данные от агентов SNMP, как показано на рисунке. На рисунке SNMP Manager, изображенный в виде ПК, подключен к маршрутизатору, коммутатору и маршрутизатору брандмауэра. Устройства, подключенные к SNMP Manager, обозначены как агенты SNMP и управляемые узлы. Стрелка, помеченная ловушкой, указывает на диспетчер SNMP, указывая, что устройства агента SNMP отправляют ловушки диспетчеру SNMP. Есть две стрелки, указывающие на агентов SNMP с надписью «Получить» и «Установить». Это означает, что диспетчер SNMP отправляет действия Get and Set агентов SNMP.
-->
...
<!--
На рисунке показано использование запроса SNMP GetRequest для определения того, находится ли интерфейс G0/0/0 в состоянии up/up. Человек за компьютерным столом говорит, что я хочу проверить переменную MIB, чтобы узнать, является ли G0/0/0. SNMP GetRequest отправляется в интерфейс R1s. R1 извлекает значение запрошенной переменной MIB.
-->
...
<!--
На рисунке показано использование ловушки SNMP для уведомления сетевого администратора о сбое интерфейса G0/0/0. На рисунке изображен сетевой администратор, сидящий за компьютером, который подключен к маршрутизатору R1. Три шага, иллюстрирующих ловушку SNMP. Первый указывает на сбой G0/0/0 маршрутизатора R1 . Во-вторых, R1 отправляет ловушку SNMP администратору сети. В-третьих, администратор сети получает текст, всплывающее окно или значок маршрутизатора становится красным на NMS GUI, уведомляя сетевого администратора о сбое интерфейса.
-->
...
<!--
Обмен всеми сообщениями SNMP проиллюстрирован на рисунке. В первом примере диспетчер SNMP отправляет запрос на получение управляемому устройству агента SNMP, которое получает значение определенной переменной MIB. Во втором примере диспетчер SNMP отправляет сообщение get-next request агенту SNMP, который извлекает следующий экземпляр переменной MIB. В третьем примере диспетчер SNMP отправляет запрос-набор агенту SNMP, который изменяет значение переменной MIB. В четвертом примере агент SNMP отправляет сообщение get-response диспетчеру SNMP, содержащее значения запрашиваемой переменной. В последнем примере агент SNMP отправляет сообщение ловушки диспетчеру SNMP, который передает незапрашевамое предупреждение.
-->
...
<!--
В анимации сетевой администратор расследует жалобу на медленный веб-сервер, отправив запрос статистики подключения агенту SNMP на веб-сервере. Агент проверяет строку сообщества и IP-адрес. Затем агент отправляет статистику на станцию управления SNMP.
-->
...
<!--
На рисунке показаны части структуры MIB, определенные Cisco. В верхней части находится .iso ниже .org затем dod, Интернет, частные, предприятия и cisco. Cisco разбивается на два столбца, один из которых имеет локальные переменные в верхней части и группу интерфейса под ним, а второй столбец имеет cisco mgmt в верхней части и cisco flash group под ним.
-->
...
<!--
На рисунке показаны статистические выборки использования ЦП маршрутизатора в течение 5 минут, которые фиксировались несколько недель. Рисунок представляет собой график использования ЦП в течение недель с 33 по 36. Слева от графика — это процент. Средняя загрузка ЦП составляет 20%, максимальная — 40%, а текущая — 36%.
-->
...
<!--
На рисунке показаны параметры команды SNMPGet. Команда snmpget -v2c, указывающая версию, -c сообщество, пароль SNMP, называемый строкой сообщества, 10.250.250.14, IP-адрес, .1.3.6.1.4.1.9.2.1.58.0, номер OID. В следующей строке отображается ответ на команду. SNMPv2-SMI::enterprises.9.2.1.58.0 = INTEGER: 11. 11 в ответе является полученным значением ЦП.
-->
...
<!--
The figure displays an example of using the Cisco SNMP navigator to research the OID information for the whyReload object on cisco.com. The figure is a screenshot of the SNMP Object Navigator translate/browse tab on cisco.com. The OID of 1.3.6.1.4.1.9.2.1.2 is typed into text box. The text box translates the OID into object name or the object name into OID to receive object details. Output is provided below the text box and gives the object information. The output on the screenshot is the Object is whyReload, the OID is 1.3.6.1.4.1.9.2.1.2, Type is DisplayString, Permission is read-only, Status is mandatory, MIB is Old-Cisco-Sys-MIB with a link to view supporting images and last is the description, this variable contains a printable octet string which contains the reason why the system was last restarted.
-->
