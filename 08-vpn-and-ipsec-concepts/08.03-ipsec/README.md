...
<!--
На рисунке изображен пользователь, отправляющий другому пользователю сто долларов через Интернет и шифрующую/дешифрующую передачу. Гейл посылает сто долларов Джереми. Используя длину ключа и алгоритм шифрования, передача зашифровывается, в результате чего сто долларов превращаются в 4ehiDx67NMop9eRU781OPotVBn45TR. Зашифрованные данные отправляются через Интернет. Злоумышленник, перехватывающий денные, не может прочитать данные, потому что они зашифрованы, после того, как данные получены компьютером Джереми, данные расшифровываются, что приводит к получению сто долларов от Гейл.
-->
...
<!--
На рисунке показана разная длина ключа и сила различных алгоритмов шифрования. Различные алгоритмы шифрования для обеспечения конфиденциальности: DES, 3DES, AES или SEAL. Длина ключа для DES составляет 56 бит, это наименее безопасный алгоритм шифрования. 3DES является вторым наименее безопасным алгоритмом шифрования, его длина ключа составляет 56 бит по 3 раза. AES более безопасен, его длина ключа 128, 192 и 256 битов. Другой более безопасный вариант — SEAL, он имеет длину ключа 160 бит. 
-->
...
<!--
На рисунке показаны различные алгоритмы хеширования и их длина ключа. Различные алгоритмы хеширования - MD5 и SHA. Наименее безопасен из двух MD5 с 128-битным ключом. SHA - более безопасен с длиной ключа 160 бит.
-->
...
<!--
На рисунке показан пример аутентификации PSK. На локальном устройстве ключ аутентификации и идентификационная информация отправляются через алгоритм хеширования, чтобы сформировать хеш. Хеш затем шифруется с помощью закрытого ключа локального устройства, что приводит к цифровой подписи. Цифровой сертификат устройства и цифровая подпись отправляются на удаленное устройство. Удаленное устройство расшифровывает цифровую подпись с помощью открытого ключа локального устройства, в результате чего получается хеш. Удаленное устройство независимо создает и вычисляет один и тот же хеш, чтобы сравнить полученный и вычисленный хеш. Хеши совпадают, и локальное устройство проходит проверку подлинности.
-->
