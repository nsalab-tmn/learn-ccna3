...
Теперь, когда вы собрали документацию, знаете методы устранения неполадок и программные и аппаратные средства для диагностики проблем, можно начать поиск и устранение неисправностей. В этом разделе рассматриваются наиболее распространенные проблемы при устранении неполадок в сети.

Во многих случаях проблемы в сети проявляются как проблемы с производительностью. Это значит, что между ожидаемым и наблюдаемым режимом работы есть разница и система не функционирует должным образом. Отказы и неоптимальные состояния на физическом уровне не только создают неудобства пользователям, но и могут отрицательно влиять на производительности всей компании. Сети, где возникают подобные ситуации, обычно перестают работать. Так как верхние уровни модели OSI зависят от функционирования физического уровня, у администратора должна быть возможность эффективно изолировать и устранять проблемы на этом уровне.
...
| **Ограниченная пропускная способность или перегрузка сети**  | <ul><li>Если маршрутизатор, интерфейс или кабель выходит из строя, протоколы маршрутизации могут перенаправить трафик на другие маршруты, которые не рассчитаны на обработку дополнительного трафика. Это приводит к заторам или появлению</li><li>«узких мест» в таких зонах.</li></ul> |
...
| **Отказы оборудования**  | <ul><li>Неисправные платы сетевых интерфейсов (NIC) могут быть причиной ошибок при передаче данных в сети вследствие коллизий, коротких кадров и некорректных сигналов.</li><li> Некорректный сигнал (Jabber) часто определяется как состояние, при котором сетевое устройство непрерывно передает беспорядочные и бессмысленные данные в сеть.</li><li>Другие вероятные причины таких сигналов — поврежденные файлы драйверов сетевых карт, плохие кабельные соединения или проблемы с заземлением.</li></ul> |
...
| **Сеть не достигает базового показателя производительности**  | <ul><li>В сети могут возникнуть два типа не оптимальных операций уровня 2 </li><li>Во-первых, передача кадров по неоптимальному пути к месту назначения приводит к неожиданно высокому потреблению пропускной способности каналов передачи данных. </li><li>Во-вторых, некоторые кадры отбрасываются, как определенные через статистику учета ошибок и через консольные сообщения на коммутаторе или маршрутизаторе. </li><li>Расширенный или непрерывный ping может помочь определить, начинают ли кадры отбрасываться.</li></ul> |
...
| **Выбор протокола транспортного уровня**  | <ul><li>После настройки ACL важно правильно указать протоколы транспортного уровня. </li><li>Многие сетевые администраторы, не зная точно, какой порт (TCP или UDP) используется конкретным потоком трафика, настраивают оба порта. </li><li>В этом случае в межсетевом экране появляется «дырка», которой могут воспользоваться злоумышленники для проникновения в сеть. </li><li>Кроме того, в списке контроля доступа также появляется дополнительный элемент, поэтому время обработки в списке контроля доступа возрастает, что приведет к увеличению задержки при передаче данных в сети.</li></ul> |
...
| **DNS**  | <ul><li>Так как маршрутизатор, на котором выполняется динамическое преобразование NAT, регулярно меняет взаимосвязь между внутренними и внешними адресами (по мере удаления и повторного создания записей в таблице), у сервера DNS за пределами маршрутизатора NAT нет точной информации о сети внутри маршрутизатора. </li><li>Настройка помощника (helper) для IPv4 может помочь решить эту проблему.</li></ul> |
