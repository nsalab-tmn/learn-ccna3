...
В предыдущем модуле вы узнали о возможностях ACL и почему это важно. В этом разделе вы узнаете о создании списков ACL.

Все списки контроля доступа должны быть спланированы. Особенно списки ACL, требующие несколько записей контроля доступа (ACE).

При настройке сложного списка ACL рекомендуется:

* используя текстовый редактор, выписать определенную политику, которая будет реализована;
* добавить команды конфигурации IOS для выполнения этих задач;
* включить комментарии для документирования списка ACL;
* скопировать и вставить команды на устройство;
* всегда тщательно тестировать ACL, чтобы убедиться, что он правильно применяет требуемую политику.
...
| `log` | <ul><li>(необязательно) это ключевое слово генерирует и отправляет информационное сообщение всякий раз, когда ACE совпадает с пакетом</li><li>сообщение включает номер ACL, совпадающее условие (permit или deny), адрес источника и количество пакетов</li><li>сообщение генерируется для первого совпадающего пакета</li><li>это ключевое слово должно быть реализовано только для устранения неполадок или обеспечения безопасности сети</li></ul> |

<!-- 5.1.3 -->
## Синтаксис стандартного именованного списка контроля доступа 
...
Имена ACL-списков состоят из буквенно-цифровых символов, они чувствительны к регистру и должны быть уникальными. Указывать имена ACL-списков заглавными буквами необязательно, но это делает их более заметными при просмотре выходных данных текущей конфигурации. Это также снижает вероятность случайного создания двух разных ACL-списков с одинаковыми именами, но различающимися использованием заглавных и строчных букв.

**Примечание**. Используйте команду глобальной конфигурации **no ip access-list** **standard** _access-list-name_ для удаления именованного стандартного ACL.

В этом примере создается именованный стандартный ACL с именем NO-ACCESS. Обратите внимание, что запрос изменяется на именованный стандартный режим конфигурации ACL. Оператора ACE вводятся в именованном стандартном режиме конфигурации ACL. Используйте справку для просмотра всех именованных стандартных параметров ACE ACL.
...
<!-- 5.1.4 -->
## Применение стандартного списка контроля доступа 

После настройки стандартного списка ACL он должен быть связан с интерфейсом или сервисом. Следующая команда может быть использована для привязки нумерованного или именованного стандартного ACL к интерфейсу:

```
Router(config-if) # ip access-group {access-list-number | access-list-name} {in | out}
```

Чтобы удалить список ACL из интерфейса, сначала введите команду конфигурации интерфейса **no ip access-group**. Однако ACL по-прежнему будет настроен на роутере. Чтобы удалить его, используйте команду глобальной конфигурации **no access-list**.

<!-- 5.1.5 -->
## Примеры стандартного нумерованного списка контроля доступа 

Топология на рисунке будет использоваться для демонстрации настройки и применения нумерованных и именованных стандартных списков ACL к интерфейсу. В первом примере показана реализация стандартного списка ACL с нумерацией.
...
Обратите внимание, что выходные данные команды **show access-lists** не отображают инструкции **remark**. Комментарии ACL отображаются в файле текущей конфигурации. Хотя команда **remark** не требуется для включения списка контроля доступа, она настоятельно рекомендуется для целей документации.

Теперь предположим, что новая политика сети гласит, что узлам в локальной сети 2 также должны быть разрешен доступа в Интернет. Чтобы включить эту политику, в список ACL 10 можно добавить вторую запись в стандартный ACL, как показано в выходных данных.
...
Результирующая политика ACL 10 разрешает только узлу 192.168.10.10 и всем узлам из LAN 2 выходить из интерфейса Serial 0/1/0. Остальные узлы в сети 192.168.10.0 не будут допущены в Интернет.

Используйте команду **show running-config** для просмотра списка ACL в конфигурации, как показано в выходных данных.
...
Обратите внимание, как также отображаются инструкции **remarks**.

Наконец, используйте команду **show ip interface**, чтобы проверить, применяется ли к интерфейсу ACL. В примере выходные данные специально просматривают интерфейс Serial 0/1/0 для строк, которые содержат текст «access list».
...
<!-- 5.1.6 -->
## Примеры стандартного именованоого списка контроля доступа 

Во втором примере показана именованная реализация стандартного ACL для . Топология повторяется на рисунке для вашего удобства.
