...
Если вы не можете запомнить настройки каждого устройства в сети, вы не одиноки. В большой сети невозможно вручную настроить каждое устройство. К счастью, есть много способов скопировать или обновить конфигурации, а затем просто вставить их. Для этого нужно знать, как просматривать файловые системы и управлять ими.

Файловая система Cisco IOS (IFS) позволяет администраторам перемещаться по различным каталогам и составлять в них списки файлов. Администратор также может создавать подкаталоги во флэш-памяти или на диске. Доступные каталоги зависят от устройства.

В примере показан результат выполнения команды **show file systems**, которая выводит список доступных файловых систем на маршрутизаторе Cisco 4221r.
...
Команда предоставляет полезную информацию, например объем общей и свободной памяти, тип файловой системы и ее разрешения. Доступные разрешения: «только чтение» (ro), «только запись» (wo) и «чтение и запись» (rw). Они отображаются в столбце Flags в выходных данных команды.

Хотя в списке доступно несколько файловых систем, нас интересуют в первую очередь TFTP, флеш-память и NVRAM.

Обратите внимание, что перед файловой системой флеш-памяти всегда стоит звездочка. Значит, флеш-память — текущая файловая система по умолчанию. В ней размещена загружаемая IOS. К списку флеш-памяти добавляется символ решетки (#), он указывает, что это загрузочный диск.
...
Поскольку флеш-память — файловая система по умолчанию, ее содержимое указывается в результатах выполнения команды **dir**. Во флеш-памяти размещено несколько файлов, однако в первую очередь нас интересует последняя запись. Это имя текущего образа файла Cisco IOS, запущенного в ОЗУ.
...
Нынешняя команда рабочего каталога **pwd** подтверждает, что просматривается именно каталог NVRAM. Команда **dir** создает список содержимого энергонезависимой памяти. В первую очередь нас интересует файл конфигурации начальной загрузки.

<!-- 10.6.2 -->
## Файловые системы коммутатора

Используя файловую систему флеш-памяти коммутатора Cisco 2960, можно скопировать файлы конфигурации и архивировать (скачивать и загружать) образы ОС.
...
**Шаг 3**. После начала процесса в командной строке привилегированного режима EXEC выполните команду **show running-config** или **show startup-config**. Текст, отображаемый в окне терминала, будет отправлен в выбранный файл.   
**Шаг 4**. Когда захват завершится, выберите **Close** в Tera Term: Окно журнала.   
**Шаг 5**. Просмотрите файл, чтобы убедиться, что он не поврежден.  

<!-- 10.6.4 -->
## Использование текстового файла для восстановления конфигурации

Конфигурацию можно скопировать из файла, а затем напрямую вставить на устройство. IOS выполняет каждую строку текста конфигурации в виде команды. Значит, файл необходимо отредактировать, чтобы зашифрованные пароли имели текстовый формат. Также необходимо удалить сообщения операционной среды IOS и весь не относящийся к командам текст типа **\--More--**. Кроме того, перед вставкой конфигурации вы можете добавить **enable** и **configure terminal** в начало файла или перейти в режим глобальной конфигурации. Этот процесс рассматривается в рамках лабораторной работы после главы.
...
**Шаг 2**. Укажите путь к файлу, который необходимо скопировать на устройство, и нажмите **Open** (Открыть).  
**Шаг 3**. После этого программа Tera Term вставит файл в память устройства.  
...
Для резервного копирования текущей конфигурации на TFTP-сервер выполните следующие действия.

**Шаг 1**. Введите команду **copy running-config tftp**.  
...
Чтобы восстановить текущую конфигурацию или конфигурацию начальной загрузки с TFTP-сервера, используйте команду **copy tftp running-config** или **copy tftp startup-config**. Выполните указанные ниже действия.
...
Далее используйте команду **copy run usbflash0:** _/_, чтобы скопировать файл конфигурации на USB. Обязательно используйте имя флеш-накопителя, указанное в файловой системе. Косую черту вводить необязательно (она обозначает корневой каталог USB-накопителя).
...
При копировании на USB-накопитель с тем же файлом конфигурации, который уже находится на диске, будет отображаться следующий вывод.
...
При наличии консольного доступа пользователь может войти в режим ROMMON, используя специальную комбинацию клавиш во время загрузки или вынув внешнюю флеш-память, когда устройство отключено. При успешном выполнении отображается подсказка **rommon 1 >**, как показано в примере.
...
Программное обеспечение ROMMON поддерживает ряд основных команд, например **confreg**. Команда **confreg 0x2142** позволяет установить для регистра конфигурации значение 0x2142. Если оно равно 0x2142, устройство будет игнорировать файл загрузочной конфигурации во время запуска. В файле хранятся забытые пароли. Изменив значение регистра конфигурации на 0x2142, введите **reset** в командной строке, чтобы перезапустить устройство. Во это время и пока распаковывается IOS введите комбинацию клавиш прерывания текущего процесса. В примере показан экран терминала маршрутизатора 1941, который вошел в режим ROMMON после отправки сигнала "break" во время процесса загрузки.
...
**Примечание:** Пароль **cisco** — ненадежный и используется здесь только для примера.
...
После настройки новых паролей измените значение регистра конфигурации обратно на 0x2102 с помощью команды **config-register 0x2102** в режиме глобальной конфигурации. Сохраните текущую конфигурацию в файле, как показано в примере.
...
Перезагрузите устройство, как показано в примере. Теперь для аутентификации используется новый пароль. С помощью команд **show** убедитесь, что все конфигурации сохранены. Например, что после восстановления пароля не отключены необходимые интерфейсы.
