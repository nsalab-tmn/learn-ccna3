...
Обратите внимание, что перед файловой системой флеш-памяти всегда стоит звездочка. Значит, флеш-память — текущая файловая система по умолчанию. Загружаемая IOS размещена во флеш-памяти. В списку флеш-памяти добавляется символ решетки (#), он указывает, что это загрузочный диск.
...
Поскольку флеш-память — файловая система по умолчанию, в результатах выполнения команды **dir** указывается содержимое флеш-памяти. Во флеш-памяти размещено несколько файлов, однако в первую очередь нас интересует последняя запись. Это имя текущего образа файла Cisco IOS, запущенного в ОЗУ.
...
Далее используйте команду **copy run usbflash0:** _/_, чтобы скопировать файл конфигурации на USB-накопитель. Обязательно используйте имя флеш-накопителя, указанное в файловой системе. Косую черту вводить необязательно (она обозначает корневой каталог USB-накопителя).
