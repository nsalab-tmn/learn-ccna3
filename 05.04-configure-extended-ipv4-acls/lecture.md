...
| `source-wildcard` | <ul><li> 32-битная шаблонная маска должна применяться к адресу источника (опционально). |
...
| `destination-wildcard` | <ul><li>32-битная шаблонная маска должна применяться к адресу получателя (опционально). |
...
| `port` | <ul><li>Десятичный номер или имя порта TCP или UDP (опционально). |
...
Выбор _protocol_ влияет на параметры _port_. Например, выбрав:
