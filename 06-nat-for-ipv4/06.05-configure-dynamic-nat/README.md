...
<!--
На рисунке изображен перевод «один к одному» для NAT. Два компьютера во внутренней сети подключены к R1 в двух сетях 192.168.10.0/24 и 192.168.11.0/24. Один компьютер с адресом 192.168.101.0, а другой — 192.168.11.10. R1 подключается через соединение S0/1/0 к R2. Затем R2 подключается через S0/1/1 к Интернету и серверу с адресом 209.165.200.254. Динамический NAT используется для преобразования между внутренней и внешней сетью через R2.
-->
...
Эта настройка используется роутером, чтобы определить, какие устройства (**list**) получают какие адреса (**pool**). В этом случае привязать NAT-POOL1 с ACL 1.
...
<!--
На рисунке изображено преобразование «один к одному». Два компьютера во внутренней сети подключены к R1 в двух сетях 192.168.10.0/24 и 192.168.11.0/24. Один компьютер имеет адрес 192.168.101.0, а другой — 192.168.11.10. R1 подключается через соединение S0/1/0 к R2. Затем R2 подключается через S0/1/1 к Интернету и серверу с адресом 209.165.200.254. Динамический NAT используется для преобразования между внутренней и внешней сетью через R2.
-->
...
5.  (a) Получив пакет с IPv4-адресом назначения 209.165.200.226, R2 выполняет поиск в таблице NAT. С помощью сопоставления из таблицы R2 выполняет преобразование адреса обратно во внутренний локальный адрес 192.168.10.10 и пересылает пакет ПК 1.
    (b) Получив пакет с IPv4-адресом назначения 209.165.200.227, R2 выполняет поиск в таблице NAT. С помощью сопоставления из таблицы R2 выполняет преобразование адреса обратно во внутренний локальный адрес 192.168.11.10 и пересылает пакет ПК 2.
6.  ПК 1 с адресом 192.168.10.10 и ПК 2 с адресом 192.168.11.10 получают пакеты и продолжают диалог. Роутер NAT выполняет шаги 2-5 для каждого пакета (шаг 6 не приводится на рисунках).
...
Команда **show ip nat translations** отображает все настроенные статические все динамические преобразования адресов, созданные в результате обработки трафика.
...
Еще одна полезная команда **show ip nat statistics** выводит сведения о суммарном количестве активных преобразований, параметрах настройки NAT, числе адресов в пуле и числе выделенных адресов.
