...
<!--
На рисунке изображены параметры для структуры IPsec. Существует пять функций IPsec, протокол IPsec, конфиденциальность, целостность, аутентификация и Диффи-Хеллман. Каждая функция имеет доступные параметры.
недоступны. Варианты протокола IPsec: AH, ESP или ESP + AH. Параметры конфиденциальности: DES, 3DES, AES или SEAL. Варианты целостности: MD5 или SHA. Варианты аутентификации:
PSK или RSA. Варианты Диффи-Хеллмана: DH1, DH2, DH5 и т. д.
-->
...
<!--
Рисунок показывает пример SA для двух различных реализаций. В первом примере есть AH для протокола IPSec, ничего не выбрано для конфиденциальности, MD5 для целостности, PSK для аутентификации 
и DH16 для Диффи-Хеллмана. Во втором примере есть ESP для протокола IPsec, AES, выбранный для конфиденциальности, SHA для целостности, RSA для аутентификации и DH24 для Диффи-Хеллмана.
-->
...
<!--
На рисунке изображен пользователь, отправляющий другому пользователю сто долларов через Интернет и шифрующую/дешифрующую передачу. Гейл посылает сто долларов Джереми. Используя длину ключа и алгоритм шифрования,
передача зашифровывается, в результате чего сто долларов превращаются в 4ehiDx67NMop9eRU781OPotVBn45TR. Зашифрованные данные отправляются через Интернет. Злоумышленник, перехватывающий денные,
не может прочитать данные, потому что они зашифрованы, после того, как данные получены компьютером Джереми, данные расшифровываются, что приводит к получению сто долларов от Гейл.
-->
...
<!--
На рисунке показана разная длина ключа и сила различных алгоритмов шифрования. Различные алгоритмы шифрования для обеспечения конфиденциальности: DES, 3DES, AES или SEAL. Длина ключа для DES 
составляет 56 бит, это наименее безопасный алгоритм шифрования. 3DES является вторым наименее безопасным алгоритмом шифрования, его длина ключа составляет 56 бит по 3 раза. AES более безопасен, его длина ключа 128, 192 и 256
битов Другой более безопасный вариант - SEAL, он имеет длину ключа 160 бит. 
-->
...
<!--
На рисунке денежная сумма, перехваченная злоумышленником, и данные изменяются, что приводит к различным результатам хэширования. Начальная сумма у Алекса была 4ehiDx67NMop9. Она была
перехвачена, и имя было изменено на Джереми, в результате чего хеш стал 12ehqPx67NMoX. Начальный и конечный хеш различаются, 4ehiDx67NMop9 и 12ehqPx67NMoX.
-->
...
<!--
На рисунке показаны различные алгоритмы хеширования и их длина ключа. Различные алгоритмы хеширования - MD5 и SHA. Наименее безопасен из двух MD5 с 128-битным ключом. SHA - более безопасен
с длиной ключа 160 бит.
-->
...
<!--
На рисунке показан пример аутентификации PSA. На локальном устройстве ключ аутентификации и идентификационная информация отправляются через алгоритм хеширования, чтобы сформировать хеш. Хеш отправляется на
удаленное устройство. Удаленное устройство независимо создает и вычисляет один и тот же хеш, чтобы сравнить полученный и вычисленный хеш. Хеши совпадают, и локальное устройство проходит проверку подлинности.
-->
...
<!--
На рисунке показан пример аутентификации PSK. На локальном устройстве ключ аутентификации и идентификационная информация отправляются через алгоритм хеширования, чтобы сформировать хеш. Хеш затем шифруется
с помощью закрытого ключа локального устройства, что приводит к цифровой подписи. Цифровой сертификат устройства и цифровая подпись отправляются на удаленное устройство. Удаленное устройство расшифровывает цифровую подпись
с помощью открытого ключа локального устройства, в результате чего получается хеш. Удаленное устройство независимо создает и вычисляет один и тот же хеш, чтобы сравнить полученный и вычисленный хеш. Хеши совпадают, и локальное устройство проходит проверку подлинности.
-->
...
<!--
На рисунке показаны различные варианты Диффи-Хеллмана. Варианты Диффи-Хеллмана идут от наименее защищенного к наиболее безопасному: начиная с наименее безопасного DH1, далее DH2, DH5, DH14, DH15, DH16, DH19, DH20, DH21
и к самому надежному DH24.
-->
