...
Эту команду необходимо настроить на всех маршрутизаторах в домене OSPF. Обратите внимание, что значение выражается в Мбит/с; поэтому для корректировки затрат на Gigabit Ethernet используйте команду **auto-cost reference-bandwidth 1000. For** 10 Gigabit Ethernet, используйте команду **auto-cost reference-bandwidth 10000.**
...
<!--
стандартную топологию сети OSPFv2, описанную в 2.1.1, с добавлением стоимости каждого канала; стоимость всех каналов маршрутизатора — маршрутизатора — 10, а стоимость всех интерфейсов loopback — 1
-->
...
<!--
стандартная топология сети OSPFv2, используемая в этом модуле, как описано в 2.1.1, с добавлением пакетов приветствия, обмениваемых между маршрутизаторами, отправляемых каждые 10 секунд на 224.0.0.5
-->
...
**Примечание**: Примечание. Интервалы приветствия (hello) и простоя (dead) по умолчанию основаны на практических рекомендациях и могут быть изменены лишь в крайних случаях.
...
<!-- 2.4.11 -->
## Packet Tracer - Изменение OSPFv2 для одной области
