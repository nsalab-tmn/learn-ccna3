...
На рисунке R2 передает пакет DBD роутеру R1, после чего выполняются следующие действия:
...
После того, как на все пакеты LSR для данного роутера отправлен отклик, смежные устройства считаются синхронизированными и переведенными в состояние Full. Обновления (LSUs) отправляются в случаях:
